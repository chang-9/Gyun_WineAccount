...
class paging{
	...
	public function showPage(){
		...
		if($this->display_mode && ($this->page != 1)){
			...
			$this->html =  "<li class='page-item'><a hresf=\"http://{$this->url}page=1\">{$this->start_btn}</a> </li>";
		}
		if($this->is_prev){
			$go_prev = $this->start_page - 1;
			//$this->html .=  "<a href=\"http://{$this->url}page=$go_prev\">{$this->prev_btn}</a> ";
			$this->html .=  "<li class='page-item'><a href=\"http://{$this->url}page=$go_prev\">{$this->prev_btn}</a></li> ";
		}
		...
		for($i = $this->start_page; $i <= $this->end_page; $i++){
			if($i == $this->page){
				...
				$this->html .= " <li class='page-item' id='page[$i]'><a href=\"http://{$this->url}page=$i\"{$this->display_class}{$this->display_id}>{$i}</a></li> ";
			}
		}
		...
		if($this->is_next){
			...
			$this->html .= " <li class='page-item'><a href=\"http://{$this->url}page=$go_next\">{$this->next_btn}</a></li>";
    	}
    	if($this->display_mode && ($this->page != $this->total_page)){
        	//$this->html .= " <a href=\"http://{$this->url}page=$this->total_page\">{$this->end_btn}</a>";
			$this->html .= " <li class='page-item'><a href=\"http://{$this->url}page=$this->total_page\">{$this->end_btn}</a></li>";
    	}
