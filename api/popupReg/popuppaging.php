...
class paging{
	...
	public function showPage(){
		...
		if($this->display_mode && ($this->page != 1)){
			...
			$this->html =  "<li class='page-item'><a id='page[1]'>{$this->start_btn}</a> </li>";
		}
		if($this->is_prev){
		...
		for($i = $this->start_page; $i <= $this->end_page; $i++){
			if($i == $this->page){
				...
				$this->html .= " <li class='page-item'><a  id='page[$i]' >{$i}</a></li> ";
			}
		}
    	...
    	if($this->display_mode && ($this->page != $this->total_page)){
			...
			$this->html .= " <li class='page-item'><a id='page[$this->end_btn]>{$this->end_btn}</a></li>";
    	}
