...
//검색 끝
	...
	$query ="SELECT * FROM Account_member ".$searchSql." ORDER BY idx DESC LIMIT $limit, $list "; //변수에 쿼리 저장
	$conn->DBQ($query);	
	$conn->DBE(); //쿼리 실행
			  ...
			  <table class="table table-bordered table-hover table-striped">
                ...
                <tbody>
				  <?$i=0;if($result!= 0){while($row = $conn->DBF()){?>
                    <tr>
					  <input type = "hidden"id="mname_akpa[<?echo $i;?>]" value ="<?echo $row['en_name'];?>">
					  <td data-title="발주 코드"><a  id="[<?echo $i;?>]" data-dismiss="modal" aria-hidden="true"><?echo $row['en_name'];?></a></td>
					  <td class="numeric" data-title="분류"><?echo $row['email'];?></td>
					  <td class="numeric" data-title="담당자명"><?echo $row['phone_num'];?></td>
